    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        while (true) {
            ...
            if (cycle % 10 == 0) {
                ...
                // Get GC stats before
                long gcCountBefore = ManagementFactory.getGarbageCollectorMXBeans()
                        .stream().mapToLong(gc -> gc.getCollectionCount()).sum();

                System.gc();
                Thread.sleep(100); // Give GC time to run

                // Check if GC actually ran
                long gcCountAfter = ManagementFactory.getGarbageCollectorMXBeans()
                        .stream().mapToLong(gc -> gc.getCollectionCount()).sum();

                System.out.printf("GC triggered: %s (before: %d, after: %d)%n",
                        gcCountAfter > gcCountBefore ? "YES" : "NO", gcCountBefore, gcCountAfter);
            }
